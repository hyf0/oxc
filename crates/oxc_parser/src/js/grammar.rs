...
fn cover_binding_identifier<'a, C: Config>(
    ...
    p.ast.binding_pattern_binding_identifier(span, name)
}
    ...
    fn cover(expr: Expression<'a>, p: &mut ParserImpl<'a, C>) -> Self {
        match expr {
            ...
            Expression::YieldExpression(e) if e.argument.is_none() && !e.delegate => {
                let span = e.span;
                p.error(diagnostics::identifier_generator("yield", span, false));
                p.ast.binding_pattern_binding_identifier(span, "yield")
            }
            Expression::ObjectExpression(obj) => {
            ...
            Expression::AssignmentExpression(assign) => {
                ...
                p.ast.binding_pattern_assignment_pattern(assign.span, left, assign.right)
            }
...
impl<'a, C: Config> CoverGrammar<'a, ObjectExpression<'a>, C> for ObjectPattern<'a> {
    #[inline(never)]
    fn cover(expr: ObjectExpression<'a>, p: &mut ParserImpl<'a, C>) -> Self {
        let mut properties = p.ast.vec_with_capacity(expr.properties.len());
        let mut rest = None;
            ...
            match prop {
                ...
                ObjectPropertyKind::SpreadProperty(spread) => {
                    if i == len - 1 {
                        ...
                        let elem = p.ast.binding_rest_element(span, target);
                        rest = Some(p.alloc(elem));
                        if let Some(span) = p.state.trailing_commas.get(&expr.span.start) {
                            p.error(diagnostics::rest_element_trailing_comma(*span));
                        }
                    } else {
        ...
        p.ast.object_pattern(expr.span, properties, rest)
    }
}
    ...
    fn cover(property: ObjectProperty<'a>, p: &mut ParserImpl<'a, C>) -> Self {
        ...
        if property.shorthand {
            ...
            if let Some(init) = p.state.cover_initialized_name.remove(&property.span.start) {
                value = p.ast.binding_pattern_assignment_pattern(property.span, value, init.right);
            }
            p.ast.binding_property(property.span, property.key, value, true, property.computed)
        } else {
            let value = BindingPattern::cover(property.value, p);
            p.ast.binding_property(property.span, property.key, value, false, property.computed)
        }
    }
}
...
impl<'a, C: Config> CoverGrammar<'a, ArrayExpression<'a>, C> for ArrayPattern<'a> {
    #[inline(never)]
    fn cover(expr: ArrayExpression<'a>, p: &mut ParserImpl<'a, C>) -> Self {
        let mut elements = p.ast.vec_with_capacity(expr.elements.len());
        let mut rest = None;
        ...
        p.ast.array_pattern(expr.span, elements, rest)
    }
}
    ...
    fn cover(target: AssignmentTarget<'a>, p: &mut ParserImpl<'a, C>) -> Self {
        match target {
            ...
            AssignmentTarget::ArrayAssignmentTarget(target) => {
                ...
                let mut elements = p.ast.vec_with_capacity(target.elements.len());
                ...
                let rest = target.rest.map(|rest| {
                    ...
                    let elem = p.ast.binding_rest_element(rest.span, argument);
                    p.alloc(elem)
                });
                p.ast.binding_pattern_array_pattern(target.span, elements, rest)
            }
            AssignmentTarget::ObjectAssignmentTarget(target) => {
                let target = target.unbox();
                let mut properties = p.ast.vec_with_capacity(target.properties.len());
                ...
                p.ast.binding_pattern_object_pattern(target.span, properties, rest)
            }
    ...
    fn cover(target: AssignmentTargetMaybeDefault<'a>, p: &mut ParserImpl<'a, C>) -> Self {
        match target {
            AssignmentTargetMaybeDefault::AssignmentTargetWithDefault(target) => {
                ...
                p.ast.binding_pattern_assignment_pattern(target.span, left, target.init)
            }
            match_assignment_target!(AssignmentTargetMaybeDefault) => {
        ...
        match property {
            AssignmentTargetProperty::AssignmentTargetPropertyIdentifier(prop) => {
                ...
                if let Some(init) = prop.init {
                    value = p.ast.binding_pattern_assignment_pattern(prop.span, value, init);
                }
                let key = PropertyKey::StaticIdentifier(
                    p.ast.alloc_identifier_name(ident_span, ident_name),
                );
                p.ast.binding_property(prop.span, key, value, true, false)
            }
            AssignmentTargetProperty::AssignmentTargetPropertyProperty(prop) => {
                let prop = prop.unbox();
                let value = BindingPattern::cover(prop.binding, p);
                p.ast.binding_property(prop.span, prop.name, value, false, prop.computed)
            }
        }
    }
}
