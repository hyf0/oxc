...
- `src/print/value.rs` — the port of `printCommaSeparatedValueGroup` /
  ...
  operands space on both sides (each ends the glued run)
...
- The 2026-06-12 ecosystem-CI sweep (fixture
  ...
  `ValueContext::no_break`, honored by `write_function`/`write_calc`);
...
- Less top-level math fill (`((@h - @f) / 2) - @a @b` exceeding the width):
  Prettier breaks AFTER the operator and packs the trailing words;
  `LessBinaryOperation` stays verbatim here (2 hits in 409 ng-zorro files),
  so the break lands between list items instead. Matching would need
  operator pseudo-entries inside `write_comma_group`'s fill
